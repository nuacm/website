...
(function($) {
  ...
  // RODERIC: Made this format the new tags in a format more palatable to nivo slider.
  ...
  // RODERIC: Added an optional callback here. Chaining does not work when you're pulling images
  // down asynchronously. The next receiver most probably won't see the requested images. 
  $.flickr.photos = function(method, options, callback) {
    ...
    return elements.each(function(index, element) {
      $.getJSON($.flickr.url(method, options), function(data) {
        ...
        $(element).append($.flickr.thumbnail.process(photos))
  ...
  $.flickr.methods = {
    // RODERIC: Call an arbitrary flickr method.
    callMethod: function(method, options, callback) {
      $.flickr.photos(method, options, callback);
    },
